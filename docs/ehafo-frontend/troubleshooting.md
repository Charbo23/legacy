...
```mermaid
flowchart TD
    A[收到用户反馈] --> B[确认用户、时间、端、入口和版本]
    B --> C[收集 PageSpy、图片或视频]
    C --> D[按 UID 查询 tmp_data_logs]
    D --> E[合并现场与异常日志]
    E --> F{证据能否判决根因}
    F -->|能| G[最小修复并真实路径验收]
    F -->|不能| H[在候选分叉点增加观测]
...
1. 根据反馈确定用户 UID、发生时间、客户端、App 包、入口环境和可见现象；用户对原因的解释先作为假设。
2. 优先收集 PageSpy、图片和视频。在有生产权限的环境中查询线上 `kaoshi.tmp_data_logs`，用 UID 作为 `itemid`，常规单用户排查不按 `model` 过滤。
...
先区分客户端预渲染、服务端生成任务、轮询状态和截图回调四段。服务端部署与进程由架构团队处理，本手册只负责指出业务链路；仓库关系见[仓库与系统关系](./repositories.md#screenshot-server)。
